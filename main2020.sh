...
# Python project path
CHALLENGE=/home/tcastrof/others/Dep2Text

# europarl corpus path
EUROPARL=/roaming/tcastrof/europarl/corpus
...
for lng in pt en es fr ko ru
  do
    echo 'Language' $lng
    cd $CHALLENGE/data2020/mt/$lng
    cp test.de test.out
    $MOSESDIR/scripts/tokenizer/detokenizer.perl -l $lng < test.out > test.detok.out

    echo 'Save results'
    python $CHALLENGE/utils.py $CHALLENGE/data2020/mt/$lng/test.out $CHALLENGE/data2020/mt/$lng/test.json $CHALLENGE/data2020/tok
    python $CHALLENGE/utils.py $CHALLENGE/data2020/mt/$lng/test.detok.out $CHALLENGE/data2020/mt/$lng/test.json $CHALLENGE/data2020/detok
  done

for lng in pt en es fr #ko ru #ar hi id ja ko ru zh
  do
    echo 'Language' $lng
    cd $CHALLENGE/data2020/mt/$lng
